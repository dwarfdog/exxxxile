    ...
    def ListPlanets():
        ...
        with connection.cursor() as cursor:
            ...
            for re in res:
                ...
                planet['credits'] = re[37] + (re[38] / 2) # - (re["upkeep"] / 24)
                if planet['credits'] < 0:
            ...
            for re in res:
                ...
                if re[19] or re[11] > 0:
                    ...
                    building = {
                        ...
                        "hydro_prod": re[17],
                        "energy_prod": re[18],
                        "ore_modifier": float(re[16]*(gcontext['planetinfo']['OreBonus']-100)/100),
                        "hydro_modifier": float(re[17]*(gcontext['planetinfo']['HydroBonus']-100)/100),
                        "energy_modifier": float(re[18]*(gcontext['planetinfo']['EnergyBonus']-100)/100),
...
def chat(request):
    ...
    def getChatId(id):
        ...
        if id = 0 and not IsNull(AllianceId) then
            ...
            if getChatId = "" or not IsNumeric(getChatId) then
                ...
                if not oRs.EOF then
                    Application("alliancechat_" & AllianceId) = re[0]
                    getChatId = re[0]
    ...
    '   end if
        ...
        if not IsEmpty(oRs) then
            while not oRs.EOF
                session("lastchatmsg_" & chatid) = re[0]
                content.AssignValue "lastmsgid", re[0]
                content.AssignValue "datetime", re[1].value
                content.AssignValue "author", re[3]
                content.AssignValue "line", re[4]
                content.AssignValue "alliancetag", getAllianceTag(re[2])
        ...
        if refresh_userlist then
            ...
            set oRs = oConn.Execute(query)
            while not oRs.EOF
                content.AssignValue "alliancetag", getAllianceTag(re[0])
                content.AssignValue "user", re[1]
                content.AssignValue "lastactivity", re[2]
    ...
    def displayChatList():
        ...
        set oRs = oConn.Execute(query)
        while not oRs.EOF
            content.AssignValue "name", re[0]
            content.AssignValue "topic", re[1]
            content.AssignValue "online", re[2]
    ...
    def displayChat():
        ...
        set oRs = oConn.Execute(query)
        while not oRs.EOF
            if addChat(re[0]) then
                content.AssignValue "id", re[0]
                content.AssignValue "name", re[1]
                content.AssignValue "topic", re[2]
                content.Parse "join"
                Session("lastchatmsg_" & re[0]) = ""
            end if
            oRs.MoveNext
        wend
            ...
            if chatid:
                ...
                if re:
                    ...
                    if re:
                        ...
                        gcontext["name"] = re[0]
                        gcontext["topic"] = re[1]
                        gcontext["join"]["setactive"] = True
                        request.session["lastchatmsg_" + str(chatid)] = ""
                else:
                    gcontext["join_error"] = True
            else:
